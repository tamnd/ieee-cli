...
describes what users get. Pick a format with `-o`, or let ieee choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ieee <command> -o table   # aligned columns for reading
ieee <command> -o jsonl   # one JSON object per line, for piping
ieee <command> -o json    # a single JSON array
ieee <command> -o csv     # spreadsheet friendly
ieee <command> -o tsv     # tab-separated
ieee <command> -o url     # just the URL column
ieee <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ieee <command> --fields id,title,url
```
...
```bash
ieee <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ieee <command>            # a table, because this is a terminal
ieee <command> | wc -l    # JSONL, because this is a pipe
```
